...
public class TestText {
    ...
    public static final String s6 = "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Maecenas porttitor congue massa. Fusce posuere, magna sed pulvinar ultricies, purus lectus malesuada libero, sit amet commodo magna eros quis urna.";

    private static void TEST_zh_CN_Typography() {
        zh_CN_Typography typography = new zh_CN_Typography(new AwtRuler());
        for (int i = 300; i > 2; i--) {
            List<String> p0 = typography.lineBreak(s0, i);
            List<String> p1 = typography.lineBreak(s1, i);
            List<String> p2 = typography.lineBreak(s2, i);
            List<String> p3 = typography.lineBreak(s3, i);
            List<String> p4 = typography.lineBreak(s4, i);
            List<String> p5 = typography.lineBreak(s5, i);
            List<String> p6 = typography.lineBreak(s6, i);
            ...
            for (String s : all)
                if (aRuler.measureWidth(s) != i) {
                    System.out.println(s);
                    flag = true;
                }
            if (flag)
                throw new RuntimeException("zh_CN_Typography未能如期断行！" + i);
        }
        final int expectWidth = 28;
        ...
        List<String> p6 = typography.lineBreak(s6, expectWidth);
        List<String> all = new ArrayList<>();
        all.addAll(p0);
        all.addAll(p1);
        all.addAll(p2);
        ...
        if (flag)
            throw new RuntimeException("zh_CN_Typography未能如期断行！" + expectWidth);
    }
}
