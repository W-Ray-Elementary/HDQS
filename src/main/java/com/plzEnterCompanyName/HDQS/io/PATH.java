...
public class PATH {
    ...
    static {
        ...
        if (runInIDEA.exists()) {
            ABSOLUTE_ROOT_PATH = runInIDEA.getAbsolutePath().substring(0,
                    runInIDEA.getAbsolutePath().lastIndexOf("\\") - 3);
        } else if (runInCmd.exists()) {
            ABSOLUTE_ROOT_PATH = runInCmd.getAbsolutePath().substring(0,
                    runInCmd.getAbsolutePath().lastIndexOf("\\") - 3);
        } else {
