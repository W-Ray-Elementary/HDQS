...
public class Configurations {
    ...
    /**
     * <p>
     * 用于处理多个同类型的{@code Configration}。
     * </p>
     * <p>
     * 通常情况下，Configration实例内部可以放入若干个子集，从而形成嵌套结构。子
     * 集们有自己的{@code name}属性，用于区分不同的子集类型。但当遇到该属性重复时，即单
     * 个{@code Configration}中拥有多个同名子集，这时传统的{@code getSubs()}的使用较为
     * 不便。此方法便是解决此问题的。
     * </p>
     * <p>
     * 一般而言，调用{@link Configration#getName()}方法的返回值称为这个Configration实例
     * 的名字。
     * </p>
     * <p>
     * 该方法要求传入多个{@code Configration}，传入值{@code moduleType}代表同名子集的名
     * 字，还要求子集中必须有一个key为{@code name}的peoperty，满足上述条件的子集会缓存在一个
     * 集合中以待查询。随后，该方法遍历这个集合，逐个检测子集中key为{@code name}的peoperty
